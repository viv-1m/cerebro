...
public class CapitalGainsReportService {
     ...
     * by quarter: {@code {"q1": [...], "q2": [...], "q3": [...], "q4": [...]}}.
     * All four keys are always present, even if a quarter has no trades.
     * Within a quarter, rows are consolidated to one entry per unique
     * security+type+term+broker combination (so at most 4 rows per stock
     * per quarter: {zerodha, groww} x {Short Term, Long Term} — fewer for
     * gold, see {@link #isGoldRelated(String)}).
     */
    public Map<String, List<CapitalGainRow>> generateConsolidatedReport(List<MultipartFile> files) throws IOException {
        Map<Key, BigDecimal> consolidated = new LinkedHashMap<>();
        Map<Key, CapitalGainRow> keyToRowMap = new LinkedHashMap<>();
        ...
        for (MultipartFile file : files) {
            ...
            try (InputStream is = file.getInputStream();
                ...
                List<CapitalGainRowWithQuarter> rows = switch (provider) {
                ...
                for (CapitalGainRowWithQuarter rwq : rows) {
                    CapitalGainRow row = rwq.row();
                    Key key = new Key(row.dematProvider(), row.securityName(),
                            row.securityType(), row.tradeTerm(), rwq.quarter());
                    consolidated.merge(key, row.profitLossAmount(), BigDecimal::add);
                    keyToRowMap.putIfAbsent(key, row);
                }
            }
        }

        // Group by quarter while building final rows
        Map<String, List<CapitalGainRow>> byQuarter = new LinkedHashMap<>();
        byQuarter.put("q1", new ArrayList<>());
        byQuarter.put("q2", new ArrayList<>());
        byQuarter.put("q3", new ArrayList<>());
        byQuarter.put("q4", new ArrayList<>());

        consolidated.entrySet().stream()
                .map(e -> new AbstractMap.SimpleEntry<>(
                        e.getKey().quarter().toLowerCase(Locale.ROOT),
                        new CapitalGainRow(
                                ...
                                e.getKey().provider())))
                .sorted(Comparator.comparing((AbstractMap.SimpleEntry<String, CapitalGainRow> e) ->
                        e.getValue().dematProvider())
                        .thenComparing(e -> e.getValue().securityName()))
                .forEach(e -> byQuarter.get(e.getKey()).add(e.getValue()));
    ...
    private List<CapitalGainRowWithQuarter> parseZerodha(Workbook workbook) {
        List<CapitalGainRowWithQuarter> results = new ArrayList<>();
        ...
        for (int r = 0; r <= sheet.getLastRowNum(); r++) {
            ...
            String tradeTerm = switch (currentSection) {
                ...
                default -> { // Non-equity / Mutual Funds: no explicit sub-section, derive from holding period
                    ...
                    // For gold items: > 3 years (1095 days) = Long Term, else Short Term
                    // For other items: > 1 year (365 days) = Long Term, else Short Term
                    if (isGoldRelated(symbol)) {
                        yield (days != null && days > 1095) ? "Long Term" : "Short Term";
                    } else {
                        yield (days != null && days > 365) ? "Long Term" : "Short Term";
                    }
                }
            };
    ...
    private List<CapitalGainRowWithQuarter> parseGroww(Workbook workbook) {
        List<CapitalGainRowWithQuarter> results = new ArrayList<>();
        ...
        for (int r = 0; r <= sheet.getLastRowNum(); r++) {
            ...
            LocalDate buyDate = parseDate(sheet, r, 3);
            ...
            // For gold items: > 3 years (1095 days) = Long Term, else Short Term
            if (isGoldRelated(stockName) && buyDate != null && sellDate != null) {
                long daysBetween = java.time.temporal.ChronoUnit.DAYS.between(buyDate, sellDate);
                tradeTerm = daysBetween > 1095 ? "Long Term" : "Short Term";
            }
    ...
    /**
     * Business rule (per your instruction): gold ETFs/units are always
     * treated as Short Term regardless of actual holding period, overriding
     * both the section-based (Groww) and holding-period-based (Zerodha
     * non-equity) term detection above.
     */
